document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('roleTable', () => ({
        ...
        getActionButtons(roleId, name) {
            return `
                <div class="flex items-center gap-2">
                    <button class="btn btn-sm btn-outline-primary update-btn" data-id="${roleId}" data-name="${name}" title="${Alpine.store('i18n').t('update')}">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                    </button>
                    <button class="btn btn-sm btn-outline-danger delete-btn" data-id="${roleId}" title="${Alpine.store('i18n').t('delete')}">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        getActionButtons1(roleId, permissionsCount) {
            return `
                <div class="flex items-center">
                    <button class="btn btn-sm btn-outline-info permissions-btn flex items-center gap-1.5" data-id="${roleId}" data-name="${this.tableData.find(r => r.id == roleId)?.name}" title="${Alpine.store('i18n').t('permissions')}">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
