document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('termsConditionsTable', () => ({
        tableData: [],
        datatable1: null,
        _initialized: false,
        ...
        async fetchTermsConditions() {
            try {
                ...
                coloredToast('danger', error.message || Alpine.store('i18n').t('failed_to_load_terms_conditions'));
                loadingIndicator.showEmptyState();
            } finally {
        ...
        async deleteTermsConditions(id) {
            try {
                const isConfirmed = await Swal.fire({
                    ...
                    text: Alpine.store('i18n').t('delete_terms_conditions_warning') || 'This action cannot be undone.',
                    icon: 'warning',
                ...
                await ApiService.deleteTermsConditions(id);
                coloredToast('success', Alpine.store('i18n').t('terms_conditions_deleted_success') || 'Terms & Conditions deleted successfully');
                await this.fetchTermsConditions();
                loadingIndicator.hide();
            } catch (error) {
                loadingIndicator.hide();
                coloredToast('danger', error.message || Alpine.store('i18n').t('failed_to_delete_terms_conditions'));
            }
        },
        ...
        openUpdateModal(id) {
            ...
            if (!item || !item.id) {
                coloredToast('danger', Alpine.store('i18n').t('terms_conditions_not_found'));
                return;
            }

            // Find modal element - try multiple selectors
            let modalElement = document.querySelector('[x-data*="updateTermsConditionsModal"]');
            
            // If not found, try to find by data attribute or class
            if (!modalElement) {
                modalElement = document.querySelector('[x-data="updateTermsConditionsModal"]');
            }
            ...
            if (!modalElement) {
                ...
                if (modalsContainer && typeof Alpine !== 'undefined') {
                    ...
                    setTimeout(() => {
                        modalElement = document.querySelector('[x-data*="updateTermsConditionsModal"]') || 
                                      document.querySelector('[x-data="updateTermsConditionsModal"]');
                        if (modalElement) {
    ...
    // Create Form
    Alpine.data('createTermsConditionsForm', () => ({
        isSubmitting: false,
        formData: {
            title: '',
            content: '',
        ...
        async submitCreate() {
            try {
                ...
                const response = await ApiService.createTermsConditions(data);
                ...
                if (response && (response.success || response.data || response.status === 'success')) {
                    coloredToast('success', Alpine.store('i18n').t('terms_conditions_created_success') || 'Terms & Conditions created successfully');
                    this.resetForm();
                    
                    // Refresh table
                    const table = Alpine.$data(document.querySelector('[x-data="termsConditionsTable"]'));
                    if (table) {
                        await table.fetchTermsConditions();
                    }
                } else {
                    throw new Error(response?.message || Alpine.store('i18n').t('failed_to_create_terms_conditions'));
                }
            } catch (error) {
                coloredToast('danger', error.message || Alpine.store('i18n').t('failed_to_create_terms_conditions'));
            } finally {
    ...
    // Update Modal
    Alpine.data('updateTermsConditionsModal', () => ({
        isOpen: false,
        isSubmitting: false,
        currentId: null,
        originalPoints: [],
        ...
        async submitUpdate() {
            try {
                ...
                const response = await ApiService.updateTermsConditions(this.currentId, data);
                ...
                if (response && (response.success || response.data || response.status === 'success' || response.status === 200 || (response.status >= 200 && response.status < 300))) {
                    coloredToast('danger', Alpine.store('i18n').t('terms_conditions_updated_success') || 'Terms & Conditions updated successfully');
                    this.closeModal();
                    
                    // Wait a bit before refreshing to ensure modal is closed
                    setTimeout(async () => {
                        // Refresh table
                        const tableElement = document.querySelector('[x-data="termsConditionsTable"]');
                        if (tableElement) {
                            const table = Alpine.$data(tableElement);
                            if (table && typeof table.fetchTermsConditions === 'function') {
                                await table.fetchTermsConditions();
                            }
                        }
                    }, 300);
                } else {
                    throw new Error(response?.message || Alpine.store('i18n').t('failed_to_update_terms_conditions'));
                }
            } catch (error) {
                coloredToast('danger', error.message || Alpine.store('i18n').t('failed_to_update_terms_conditions'));
            } finally {
                this.isSubmitting = false;
                loadingIndicator.hide();
            }
