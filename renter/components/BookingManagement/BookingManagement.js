    ...
    Alpine.data('bookingTable', () => ({
        ...
        populateTable() {
            ...
            const mappedData = this.tableData.map((booking, index) => [
                this.formatText(booking.id),
                this.formatCarInfo(booking.car_details),
            ...
            return `
                            <div class="flex items-center w-max" x-data="{ imgError: false }">
                                <img class="w-9 h-9 rounded-full ltr:mr-2 rtl:ml-2 object-cover"
                                    ...
                                    width="36"
                                    height="36" />
                                ${carDetails.make || 'Car'} (${carDetails.year || 'N/A'})
                            </div>`;
        },
        ...
        getActionButtons(bookingId, status, isPaid) {
            return `
                            <div class="flex items-center gap-1 justify-center">
                                ...
                                <button class="btn view-car-btn btn-primary btn-sm" data-id="${bookingId}">
                                    ${Alpine.store('i18n').t('view_details')}
                                </button>

                                <!-- Change Status Button -->
                                <button class="btn change-status-btn btn-warning btn-sm" data-id="${bookingId}">
                                    ${Alpine.store('i18n').t('change_status')}
                                </button>

                                <!-- Change Payment Status Button -->
                                <button class="btn change-payment-btn ${isPaid === '1' ? 'btn-success' : 'btn-danger'} btn-sm" data-id="${bookingId}">
        ...
        async showCarDetails(bookingId) {
            try {
                ...
                if (!booking || !booking.car_details) {
        ...
        async changePaymentStatus(bookingId) {
            try {
                const { value: isPaid } = await Swal.fire({
                    ...
                    inputValidator: (value) => {
                        if (value === undefined) {
                            return 'You need to select a payment status!';
                        }
                    }
                });
