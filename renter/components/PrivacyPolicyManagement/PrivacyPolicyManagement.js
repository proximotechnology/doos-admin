document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('privacyPolicyTable', () => ({
        tableData: [],
        datatable1: null,
        _initialized: false,
        ...
        async fetchPrivacyPolicy() {
            try {
                ...
                coloredToast('danger', error.message || Alpine.store('i18n').t('failed_to_load_privacy_policy'));
                loadingIndicator.showEmptyState();
            } finally {
        ...
        async deletePrivacyPolicy(id) {
            try {
                const isConfirmed = await Swal.fire({
                    ...
                    text: Alpine.store('i18n').t('delete_privacy_policy_warning') || 'This action cannot be undone.',
                    icon: 'warning',
                ...
                await ApiService.deletePrivacyPolicy(id);
                coloredToast('success', Alpine.store('i18n').t('privacy_policy_deleted_success') || 'Privacy Policy deleted successfully');
                await this.fetchPrivacyPolicy();
                loadingIndicator.hide();
            } catch (error) {
                loadingIndicator.hide();
                coloredToast('danger', error.message || Alpine.store('i18n').t('failed_to_delete_privacy_policy'));
            }
        },
        ...
        openUpdateModal(id) {
            ...
            if (!item || !item.id) {
                coloredToast('danger', Alpine.store('i18n').t('privacy_policy_not_found'));
                return;
            }

            // Find modal element - try multiple selectors
            let modalElement = document.querySelector('[x-data*="updatePrivacyPolicyModal"]');
            
            // If not found, try to find by data attribute or class
            if (!modalElement) {
                modalElement = document.querySelector('[x-data="updatePrivacyPolicyModal"]');
            }
            
            // If still not found, try to initialize Alpine on modals container
            if (!modalElement) {
                const modalsContainer = document.getElementById('privacy-policy-modals-container');
                if (modalsContainer && typeof Alpine !== 'undefined') {
                    Alpine.initTree(modalsContainer);
                    // Wait a bit for Alpine to initialize
                    setTimeout(() => {
                        modalElement = document.querySelector('[x-data*="updatePrivacyPolicyModal"]') || 
                                      document.querySelector('[x-data="updatePrivacyPolicyModal"]');
                        if (modalElement) {
    ...
    // Create Form
    Alpine.data('createPrivacyPolicyForm', () => ({
        isSubmitting: false,
        formData: {
            title: '',
            content: '',
        ...
        async submitCreate() {
            try {
                ...
                const response = await ApiService.createPrivacyPolicy(data);
                ...
                if (response && (response.success || response.data || response.status === 'success')) {
                    coloredToast('success', Alpine.store('i18n').t('privacy_policy_created_success') || 'Privacy Policy created successfully');
                    this.resetForm();
                    
                    // Refresh table
                    const table = Alpine.$data(document.querySelector('[x-data="privacyPolicyTable"]'));
                    if (table) {
                        await table.fetchPrivacyPolicy();
                    }
                } else {
                    throw new Error(response?.message || Alpine.store('i18n').t('failed_to_create_privacy_policy'));
                }
            } catch (error) {
                coloredToast('danger', error.message || Alpine.store('i18n').t('failed_to_create_privacy_policy'));
            } finally {
    ...
    // Update Modal
    Alpine.data('updatePrivacyPolicyModal', () => ({
        isOpen: false,
        isSubmitting: false,
        currentId: null,
        originalPoints: [],
        ...
        async submitUpdate() {
            try {
                ...
                const response = await ApiService.updatePrivacyPolicy(this.currentId, data);
                ...
                if (response && (response.success || response.data || response.status === 'success' || response.status === 200 || (response.status >= 200 && response.status < 300))) {
                    coloredToast('success', Alpine.store('i18n').t('privacy_policy_updated_success') || 'Privacy Policy updated successfully');
                    this.closeModal();
                    
                    // Wait a bit before refreshing to ensure modal is closed
                    setTimeout(async () => {
                        // Refresh table
                        const tableElement = document.querySelector('[x-data="privacyPolicyTable"]');
                        if (tableElement) {
                            const table = Alpine.$data(tableElement);
                            if (table && typeof table.fetchPrivacyPolicy === 'function') {
                                await table.fetchPrivacyPolicy();
                            }
                        }
                    }, 300);
                } else {
                    throw new Error(response?.message || Alpine.store('i18n').t('failed_to_update_privacy_policy'));
                }
            } catch (error) {
                coloredToast('danger', error.message || Alpine.store('i18n').t('failed_to_update_privacy_policy'));
            } finally {
                this.isSubmitting = false;
                loadingIndicator.hide();
            }
