...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.store('blackLocationsTable', {
        refreshTable: async function () {
            const tableComponent = Alpine.$data(document.querySelector('[x-data="blackLocationsTable"]'));
            if (tableComponent && tableComponent.fetchBlackLocations) {
                await tableComponent.fetchBlackLocations(tableComponent.currentPage);
            }
        },
        ...
        updateBlackLocation: async function (blackLocationId) {
            try {
                ...
                const response = await fetch(`${API_CONFIG.BASE_URL_Renter}/api/admin/black_location/update/${blackLocationId}`, {
                    method: 'PUT',
                    headers: {
                    ...
                    body: JSON.stringify({
                        address: modalComponent.blackLocationData.address,
                        lat: modalComponent.blackLocationData.lat,
                        lang: modalComponent.blackLocationData.lang,
                    }),
                });

                if (!response.ok) {
                    const errorData = await response.json().catch(() => ({}));
                    throw new Error(errorData.message || Alpine.store('i18n').t('failed_update_black_location'));
                }

                coloredToast('success', Alpine.store('i18n').t('black_location_updated_success'));
                modalComponent.open = false;
                await this.refreshTable();
            } catch (error) {
                console.error('Error updating black location:', error);
    ...
    Alpine.data('blackLocationsTable', () => ({
        tableData: [],
        paginationMeta: {
            current_page: 1,
            last_page: 1,
        ...
        async init() {
            ...
            await this.fetchBlackLocations(1);

            document.addEventListener('click', (e) => {
                if (e.target.closest('.delete-btn')) {
                    const blackLocationId = e.target.closest('.delete-btn').dataset.id;
                    this.deleteBlackLocation(blackLocationId);
                }
                if (e.target.closest('.update-btn')) {
                    const blackLocationId = e.target.closest('.update-btn').dataset.id;
                    const blackLocation = this.tableData.find(s => s.id == blackLocationId);
                    if (blackLocation) {
                        Alpine.store('blackLocationsTable').openEditModal(blackLocation);
                    }
                }
                if (e.target.closest('.pagination-btn')) {
                    const page = e.target.closest('.pagination-btn').dataset.page;
                    this.fetchBlackLocations(page);
                }
            });
        },

        async fetchBlackLocations(page = 1) {
            try {
                ...
                const response = await fetch(`${this.apiBaseUrl}/api/admin/black_location/index?${queryParams.toString()}`, {
                    method: 'GET',
                    headers: {
                        Accept: 'application/json',
                        Authorization: `Bearer ${token}`,
                ...
                if (!response.ok) {
                    throw new Error(Alpine.store('i18n').t('failed_fetch_black_locations'));
                }
                ...
                console.error('Error fetching black locations:', error);
                loadingIndicator.hideTableLoader();
                loadingIndicator.showEmptyState();
        ...
        populateTable() {
            ...
            const mappedData = this.tableData.map((blackLocation, index) => [
                this.formatText((this.currentPage - 1) * this.paginationMeta.per_page + index + 1),
                this.formatText(blackLocation.address),
                this.formatCoordinates(blackLocation.lat, blackLocation.lang),
                this.getLocationLink(blackLocation.lat, blackLocation.lang),
        ...
        async deleteBlackLocation(blackLocationId) {
            ...
            try {
                loadingIndicator.show();
                const token = localStorage.getItem('authToken');
                const response = await fetch(`${this.apiBaseUrl}/api/admin/black_location/delete/${blackLocationId}`, {
                    method: 'DELETE',
                    headers: {
                        Accept: 'application/json',
                        Authorization: `Bearer ${token}`,
                ...
                if (!response.ok) throw new Error(Alpine.store('i18n').t('failed_delete_black_location'));
    ...
    Alpine.data('Add_Black_Location', () => ({
        apiBaseUrl: API_CONFIG.BASE_URL_Renter,
        address: '',
        lat: '',
        lang: '',

        async Add_Black_Location() {
            try {
                loadingIndicator.show();
                const token = localStorage.getItem('authToken');
                if (!token) throw new Error(Alpine.store('i18n').t('auth_token_missing'));

                const response = await fetch(`${this.apiBaseUrl}/api/admin/black_location/store`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        Accept: 'application/json',
                ...
                coloredToast('success', Alpine.store('i18n').t('black_location_added_success'));
                await Alpine.store('blackLocationsTable').refreshTable();
            } catch (error) {
                coloredToast('danger', error.message);
            } finally {
                loadingIndicator.hide();
