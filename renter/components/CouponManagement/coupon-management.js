document.addEventListener('alpine:init', () => {
    const loadingIndicator = {
        ...
        showTableLoader: function () {
            ...
            if (couponsTableContainer) couponsTableContainer.style.display = 'none';
        ...
        hideTableLoader: function () {
            ...
            if (couponsTableContainer) couponsTableContainer.style.display = 'block';
        },
    ...
    Alpine.data('couponTable', () => ({
        ...
        populateTable() {
            ...
            const tableElement = document.getElementById('couponsTable');
            if (!tableElement) {
        ...
        getActionButtons(couponId, status) {
            ...
            return `
                <div class="flex items-center gap-2">
                    <button class="btn btn-sm btn-outline-primary update-btn" data-id="${couponId}" title="${Alpine.store('i18n').t('update')}">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                    </button>
                    ${isActive ?
                    `<button class="btn btn-sm btn-outline-warning deactivate-btn" data-id="${couponId}" title="${Alpine.store('i18n').t('deactivate')}">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" />
                        </svg>
                    </button>` :
                    `<button class="btn btn-sm btn-outline-success activate-btn" data-id="${couponId}" title="${Alpine.store('i18n').t('activate')}">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                    </button>`
                }
                    <button class="btn btn-sm btn-outline-danger delete-btn" data-id="${couponId}" title="${Alpine.store('i18n').t('delete')}">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
